...
function testPageWidthModifiersExist() {
  ...
  assert.match(css, /:where\(\.report-entry-page\) \.report-submit-confirm-modal-card \{[\s\S]*--report-submit-modal-title-size: clamp\(38px, 8\.6vw, 44px\);[\s\S]*--report-submit-modal-subtitle-size: clamp\(28px, 6\.8vw, 32px\);[\s\S]*--report-submit-modal-label-size: clamp\(28px, 6\.4vw, 36px\);[\s\S]*--report-submit-modal-value-size: clamp\(30px, 6\.8vw, 38px\);[\s\S]*border: var\(--line-strong\) solid var\(--modal-shell-border\) !important;[\s\S]*border-radius: var\(--radius-lg\);/);
