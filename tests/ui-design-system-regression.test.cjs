...
function testPageWidthModifiersExist() {
  ...
  assert.match(read("confirm.html"), /id="confirmGoalCard"/);
  assert.match(read("confirm.html"), /id="confirmGoalStage"/);
  assert.match(read("confirm.html"), /id="confirmGoalFill"/);
  assert.match(read("confirm.html"), /id="confirmGoalShot"/);
  assert.match(read("confirm.html"), /const PLAN_KEY = "tsms_sales_plan";/);
  assert.match(read("confirm.html"), /const GOAL_ZONE_STEPS = \[/);
  ...
  assert.match(read("confirm.html"), /flushPendingGoalAnimation\(\);/);
  ...
  assert.match(read("report.html"), /const SETTINGS_KEY = "tsms_settings";/);
  assert.match(read("report.html"), /function estimateEntryTakeHome\(entryLike\) \{/);
  assert.match(read("report.html"), /deltaTakeHome: entryTakeHome - previousEntryTakeHome/);
  ...
  assert.match(read("report.html"), /<p class="confirm-summary-modal-title report-submit-confirm-modal-title" id="submitConfirmTitle">登録内容確認<\/p>/);
  assert.match(read("report.html"), /<p class="confirm-summary-modal-subtitle report-submit-confirm-modal-subtitle" id="submitConfirmModalSubtitle" hidden><\/p>/);
  assert.match(read("report.html"), /class="confirm-summary-modal-list report-submit-confirm-modal-list"/);
  assert.match(read("report.html"), /<div class="line" id="confirmPayMethodLine"><div class="k">支払方法<\/div><div class="v value-highlight" id="confirmPayMethod">-<\/div><\/div>/);
  assert.match(read("report.html"), /<div class="line" id="confirmTicketSubLine" hidden><div class="k">チケット種別<\/div><div class="v" id="confirmTicketSub">-<\/div><\/div>/);
  assert.match(read("report.html"), /<div class="line" id="confirmDiscountLine" hidden><div class="k">障害割引<\/div><div class="v" id="confirmDiscount">なし<\/div><\/div>/);
  assert.match(read("report.html"), /<div class="line" id="confirmNoteLine" hidden><div class="k">備考<\/div><div class="v" id="confirmNote">-<\/div><\/div>/);
  assert.match(read("report.html"), /function setSubmitConfirmLineVisibility\(line, visible\) \{/);
  assert.match(read("report.html"), /setSubmitConfirmLineVisibility\(confirmTicketSubLine, !!ticketSubValue\);/);
  assert.match(read("report.html"), /setSubmitConfirmLineVisibility\(confirmDiscountLine, !!discountValue\);/);
  assert.match(read("report.html"), /setSubmitConfirmLineVisibility\(confirmNoteLine, !!noteValue\);/);
  ...
  assert.match(css, /:where\(\.confirm-page\) \.confirm-goal-card \{[\s\S]*overflow: hidden;[\s\S]*background:[\s\S]*linear-gradient\(180deg, #101827 0%, #121b31 48%, #0b1020 100%\) !important;/);
  ...
  assert.match(css, /:where\(\.confirm-page\) \.confirm-summary-modal-list \.line \{[\s\S]*display: grid;[\s\S]*grid-template-columns: 150px minmax\(0, 1fr\);[\s\S]*align-items: baseline;[\s\S]*column-gap: var\(--space-3\);/);
  ...
  assert.match(css, /:where\(\.confirm-page\) \.confirm-summary-modal-list \.k \{[\s\S]*font-size: var\(--confirm-summary-modal-label-size\) !important;[\s\S]*font-weight: 800;[\s\S]*line-height: 1\.2 !important;[\s\S]*min-width: 0;[\s\S]*white-space: nowrap;/);
  assert.match(css, /:where\(\.confirm-page\) \.confirm-summary-modal-list \.v \{[\s\S]*font-size: var\(--confirm-summary-modal-value-size\) !important;[\s\S]*font-weight: 900;[\s\S]*line-height: 1\.15 !important;[\s\S]*min-width: 0;[\s\S]*max-width: 100%;[\s\S]*justify-self: end;[\s\S]*white-space: normal;[\s\S]*overflow-wrap: anywhere;/);
