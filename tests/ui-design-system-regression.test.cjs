...
function testPageWidthModifiersExist() {
  ...
  assert.match(css, /:where\(\.report-entry-page\) \.report-submit-confirm-modal-card \{[\s\S]*--report-submit-modal-title-size: clamp\(32px, 5\.8vw, 44px\);[\s\S]*--report-submit-modal-subtitle-size: clamp\(24px, 4\.4vw, 32px\);[\s\S]*--report-submit-modal-label-size: clamp\(26px, 4\.8vw, 36px\);[\s\S]*--report-submit-modal-value-size: clamp\(28px, 5vw, 38px\);[\s\S]*width: min\(720px, 100%\);[\s\S]*padding: 28px 24px 24px;/);
  assert.match(css, /:where\(\.report-entry-page\) \.report-submit-confirm-modal-card \.confirm-summary-modal-title \{[\s\S]*font-size: var\(--report-submit-modal-title-size\);/);
  assert.match(css, /:where\(\.report-entry-page\) \.report-submit-confirm-modal-card \.confirm-summary-modal-subtitle \{[\s\S]*font-size: var\(--report-submit-modal-subtitle-size\);/);
  assert.match(css, /:where\(\.report-entry-page\) \.report-submit-confirm-modal-list \.line \{[\s\S]*gap: 18px;[\s\S]*padding: 18px 0;/);
  assert.match(css, /:where\(\.report-entry-page\) \.report-submit-confirm-modal-list \.k \{[\s\S]*width: 220px;[\s\S]*font-size: var\(--report-submit-modal-label-size\);[\s\S]*font-weight: 800;/);
  assert.match(css, /:where\(\.report-entry-page\) \.report-submit-confirm-modal-list \.v \{[\s\S]*font-size: var\(--report-submit-modal-value-size\);[\s\S]*font-weight: 900;/);
  assert.match(css, /:where\(\.report-entry-page\) \.report-submit-confirm-modal-actions \.btn \{[\s\S]*min-height: 92px;[\s\S]*font-size: var\(--report-submit-modal-button-size\);/);
  assert.match(css, /@media \(max-width: 640px\) \{[\s\S]*:where\(\.report-entry-page\) \.report-submit-confirm-modal-list \.line \{[\s\S]*flex-direction: column;[\s\S]*:where\(\.report-entry-page\) \.report-submit-confirm-modal-list \.k,[\s\S]*:where\(\.report-entry-page\) \.report-submit-confirm-modal-list \.v \{/);
